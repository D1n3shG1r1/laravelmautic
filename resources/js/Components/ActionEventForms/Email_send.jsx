...
const DecisionModalContent = (inputData) => {
  ...
  const getEventDropdownEmailsList = () => {
    
    var url = 'campaigns/getEventDropdownEmailsList';
    var postJson = {"_token":csrftoken};
    httpRequest(url, postJson, function(resp){
        if(resp.C == 100){

          if(isRealVal(resp.R)) {
            setEmailsOptions(resp.R);
          }else{
            setEmailsOptions([]);
          }
  ...
  const saveEvent = (event) =>{
      ...
      var email = document.getElementById("campaignevent_properties_email").value;
      ...
      if(!isRealVal(eventName)){
          ...
          return false;
      }else if(!isRealVal(email)){
        var err = 1;
        var msg = "Select the email to be sent.";
        showToastMsg(err, msg);
        return false;
    }else{
        //campaignevent_form
  ...
  return (
    ...
    <div className="bundle-form">
      <div className="bundle-form-header mb-10">
        <h3>Send email</h3>
        <h6 className="text-muted">Send the selected email to the contact.</h6>
      </div>
        ...
        <div className="row">
            ...
            <div id="campaignevent_properties">
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_email" data-toggle="tooltip" data-container="body" data-placement="top" title="" data-original-title="Choose the email to be sent. If your email is not displayed, it may not be published or it is a segment email.">
                        Email to send <i className="ri-question-line"></i></InputLabel>

                        <div className="choice-wrapper">
                            <select id="campaignevent_properties_email" name="campaignevent[properties][email][]" className="form-control" autoComplete="false">
                            
                            {emailsOptions.map((email) => (
                              <option
                              key={`seg_${email.id}`}
                              id={`eventEmail_${email.id}`}
                              title={`${email.name}`}
                              value={email.id}>
                                  {`${email.name}`}
                              </option>
