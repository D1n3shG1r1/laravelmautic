...
const DecisionModalContent = ({inputData, jsPlumbInstanceRef}) => {
  ...
  const getEventCampaignList = () => {
    
    var url = 'campaigns/getEventCampaignList';
    var postJson = {"_token":csrftoken};
    httpRequest(url, postJson, function(resp){
        if(resp.C == 100){

          if(isRealVal(resp.R)) {
            setCampaignsOptions(resp.R);
          }else{
            setCampaignsOptions([]);
          }
  ...
  useEffect(() => {
    if (window.jQuery) {
      ...
      $('#campaignevent_properties_addTo').select2({
        placeholder: "Select campaigns",
        allowClear: true,
        width: '100%', // Makes it responsive
      });

      $('#campaignevent_properties_removeFrom').select2({
        placeholder: "Select campaigns",
        allowClear: true,
        width: '100%', // Makes it responsive
      });
      ...
      return () => {
        $('#campaignevent_properties_addTo').select2('destroy');
        $('#campaignevent_properties_removeFrom').select2('destroy');
      };
  ...
  const saveEvent = (event) =>{
      ...
      var addTo = document.getElementById("campaignevent_properties_addTo").value;
      var removeFrom = document.getElementById("campaignevent_properties_removeFrom").value;
  ...
  return (
    ...
    <div className="bundle-form">
      <div className="bundle-form-header mb-10">
        <h3>Change campaigns</h3>
        <h6 className="text-muted">Add contact to specific campaigns and/or remove from specific campaigns when the event is triggered.</h6>
      </div>
        ...
        <div className="row">
            ...
            <div id="campaignevent_properties">
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_addTo" value="Add contact to" />
                        <select id="campaignevent_properties_addTo" name="campaignevent[properties][addTo][]" className="form-control" autoComplete="false" multiple="multiple">
                          {campaignsOptions.map((campaign) => (
                            <option
                            key={`camp_${campaign.id}`}
                            id={`eventCampaign_${campaign.id}`}
                            title={`${campaign.name}`}
                            value={campaign.id}>
                                {`${campaign.name}`}
                            </option>
                ...
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlForfor="campaignevent_properties_removeFrom" value="Remove contact from"/>
                        <select id="campaignevent_properties_removeFrom" name="campaignevent[properties][removeFrom][]" className="form-control" autoComplete="false" multiple="multiple">
                          {campaignsOptions.map((campaign) => (
                            <option
                            key={`camp_${campaign.id}`}
                            id={`eventCampaign_${campaign.id}`}
