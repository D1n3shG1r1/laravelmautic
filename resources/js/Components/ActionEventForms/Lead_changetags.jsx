...
const DecisionModalContent = ({inputData, jsPlumbInstanceRef}) => {
  ...
  const getEventDropdownTagsList = () => {
    
    var url = 'campaigns/getEventDropdownTagsList';
    var postJson = {"_token":csrftoken};
    httpRequest(url, postJson, function(resp){
        if(resp.C == 100){

          if(isRealVal(resp.R)) {
            setTagsOptions(resp.R);
          }else{
            setTagsOptions([]);
          }
  ...
  useEffect(() => {
    if (window.jQuery) {
      ...
      $('#campaignevent_properties_add_tags').select2({
        placeholder: "Select tags",
        allowClear: true,
        width: '100%', // Makes it responsive
      });

      $('#campaignevent_properties_remove_tags').select2({
        placeholder: "Select tags",
        allowClear: true,
        width: '100%', // Makes it responsive
      });
      ...
      return () => {
        $('#campaignevent_properties_add_tags').select2('destroy');
        $('#campaignevent_properties_remove_tags').select2('destroy');
      };
  ...
  const saveEvent = (event) =>{
      ...
      var addTo = document.getElementById("campaignevent_properties_add_tags").value;
      var removeFrom = document.getElementById("campaignevent_properties_remove_tags").value;
  ...
  return (
    ...
    <div className="bundle-form">
      <div className="bundle-form-header mb-10">
        <h3>Modify contact's tags</h3>
        <h6 className="text-muted">Add tag to or remove tag from contact.</h6>
      </div>
        ...
        <div className="row">
            ...
            <div id="campaignevent_properties">
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_add_tags">
                        Add tags<i className="ri-question-line"></i>
                        </InputLabel>
                        <div className="choice-wrapper">
                            <select id="campaignevent_properties_add_tags" name="campaignevent[properties][add_tags][]" className="form-control" autoComplete="false" multiple="multiple">
                            {tagsOptions.map((tag) => (
                              <option
                              key={`tagAdd_${tag.id}`}
                              id={`eventTagAdd_${tag.id}`}
                              title={`${tag.tag}`}
                              value={tag.id}>
                                  {`${tag.tag}`}
                              </option>
                ...
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_remove_tags">
                        Remove tags<i className="ri-question-line"></i>
                        </InputLabel>
                        
                        <div className="choice-wrapper">
                            <select id="campaignevent_properties_remove_tags" name="campaignevent[properties][remove_tags][]" className="form-control" autoComplete="false" multiple="multiple">
                            {tagsOptions.map((tag) => (
                              <option
                              key={`tagRmv_${tag.id}`}
                              id={`eventtagRmv_${tag.id}`}
                              title={`${tag.tag}`}
                              value={tag.id}>
                                  {`${tag.tag}`}
                              </option>
