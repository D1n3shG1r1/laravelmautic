...
const DecisionModalContent = ({inputData, jsPlumbInstanceRef}) => {
  ...
  const getEventDropdownSegmentsList = () => {
    
    var url = 'campaigns/getEventDropdownSegmentsList';
    var postJson = {"_token":csrftoken};
    httpRequest(url, postJson, function(resp){
        if(resp.C == 100){

          if(isRealVal(resp.R)) {
            setSegmentsOptions(resp.R);
          }else{
            setSegmentsOptions([]);
          }
  ...
  useEffect(() => {
    if (window.jQuery) {
      ...
      $('#campaignevent_properties_addToLists').select2({
        placeholder: "Select campaigns",
        allowClear: true,
        width: '100%', // Makes it responsive
      });

      $('#campaignevent_properties_removeFromLists').select2({
        placeholder: "Select campaigns",
        allowClear: true,
        width: '100%', // Makes it responsive
      });
      ...
      return () => {
        $('#campaignevent_properties_addToLists').select2('destroy');
        $('#campaignevent_properties_removeFromLists').select2('destroy');
      };
  ...
  const saveEvent = (event) =>{
      ...
      var addTo = document.getElementById("campaignevent_properties_addToLists").value;
      var removeFrom = document.getElementById("campaignevent_properties_removeFromLists").value;
  ...
  return (
    ...
    <div className="bundle-form">
      <div className="bundle-form-header mb-10">
        <h3>Modify contact's segments</h3>
        <h6 className="text-muted">Add contact to or remove contact from segment(s)</h6>
      </div>
        ...
        <div className="row">
            ...
            <div id="campaignevent_properties">
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_addToLists" value="Add contact to selected segment(s)" />
                        
                        <select id="campaignevent_properties_addToLists" name="campaignevent[properties][addToLists][]" className="form-control" autoComplete="false" multiple="multiple">
                          {segmentsOptions.map((segment) => (
                              <option
                              key={`segAdd_${segment.id}`}
                              id={`eventSegmentAdd_${segment.id}`}
                              title={`${segment.name}`}
                              value={segment.id}>
                                  {`${segment.name}`}
                              </option>
                ...
                <div className="row">
                    <div className="form-group col-xs-12 ">
                        <InputLabel className="control-label required" htmlFor="campaignevent_properties_removeFromLists" value="Remove contact from selected segment(s)" />
                        <select id="campaignevent_properties_removeFromLists" name="campaignevent[properties][removeFromLists][]" className="form-control" autoComplete="false" multiple="multiple">
                            {segmentsOptions.map((segment) => (
                              <option
                              key={`segRmv_${segment.id}`}
                              id={`eventSegmentRmv_${segment.id}`}
                              title={`${segment.name}`}
                              value={segment.id}>
                                  {`${segment.name}`}
                              </option>
